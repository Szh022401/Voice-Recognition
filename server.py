...
def get_evaluate_fn(val_set, test_set, encoding_fn, num_rounds):
    def evaluate(server_round: int, parameters: fl.common.NDArrays, config):
        ...
        # prepare model
        encoder, classifier = get_model(device, NUM_CLASSES, compile=False)
        ...
        if server_round == num_rounds:
            prefix = "test"
            encoded = test_set.map(encoding_fn, num_proc=1, remove_columns=remove_cols)
        else:
            prefix = "val"
            encoded = val_set.map(encoding_fn, num_proc=1, remove_columns=remove_cols)
        ...
        # Save model
        if server_round == num_rounds:
            save_model(encoder, classifier, device, 'global_model.pth')
...
def save_model(encoder, classifier, device, save_path):
    encoder_cpu = encoder.to('cpu')
    classifier_cpu = classifier.to('cpu')

    torch.save({
        'encoder_state_dict': encoder_cpu.state_dict(),
        'classifier_state_dict': classifier_cpu.state_dict(),
    }, save_path)
